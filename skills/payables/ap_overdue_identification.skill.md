---
name: ap_overdue_identification
description: Identify open payables items, compute overdue days, and assign aging buckets
version: 1.0
type: executable-skill
domain: payables
input: payables-ledger-gold
output: ap-overdue-identification-output
status: locked
---

# AP OVERDUE IDENTIFICATION SKILL

## purpose
Identify payables line items that are open, overdue, or not yet due.

Compute overdue days and assign aging buckets at line-item level.

This skill must not:
- compute cash preservation logic
- classify payment strategy
- assign actions
- aggregate totals
...
- step: validate_input
  ...
  on_failure:
    - action: terminate
      error_code: INVALID_PAYABLES_LEDGER_INPUT
...
- rule: no_cash_preservation_logic description: do not compute payment optimization or cash strategy
- rule: no_issue_classification description: do not classify early payment, delay, or risk
- rule: no_aggregation description: do not aggregate totals
...
## failure_modes
- code: MISSING_DATASET condition: payables-ledger-gold not provided action: terminate
- code: INVALID_STRUCTURE condition: input dataset fails contract validation action: terminate
- code: NO_OPEN_ITEMS condition: no rows remain after filtering action: return_empty_output
- code: INVALID_DATE_FIELDS condition: due_date or evaluation_date invalid action: terminate

## notes
- this skill operates only at line-item level
- this skill is the only source of overdue_days and aging_bucket for payables workflow
- downstream skills must use this output and must not recompute aging
